...
class DistributedTaskClient:
    ...
    async def call_remote_agent(self, url, instruction, timeout=300):
        """调用远程代理"""
        try:
            async with httpx.AsyncClient(timeout=timeout, trust_env=False) as client:
                ...
                response = await client.post(
                    f"{url}/api/chat/stream", json=payload, timeout=timeout
                )
                ...
                if response.status_code == 200:
                            ...
                            try:
                                if current_event == "message":
                                    ...
                                    if event_type == "answer_chunk":
                                        ...
                                        result_text += content
                                        chunk_count += 1
                                        if chunk_count <= 5:  # 只打印前 5 个 chunk
                                        ...
                                        print(
                                            f"  ❌ Error: {event_data.get('content', '')}"
                                        )
                    ...
                    print(f"  📥 响应完成: {line_count} 行, {chunk_count} 个 chunk")
    ...
    async def execute_distributed_task(self):
        ...
        if root_caps:
            ...
            print(f"  📊 可用Agent数量: {len(root_caps.get('tools', []))}")
        else:
        ...
        for name, url in self.sub_nodes.items():
            ...
            if caps:
                print(f"  ✅ {name} 能力树已加载 ({len(caps.get('tools', []))} 工具)")
            else:
        ...
        task_instruction = (
            "使用qwen_bridge_agent执行命令ps aux并统计进程数量，返回精确的数字"
        )
        ...
        for name, url in self.sub_nodes.items():
            ...
            print(f"🎯 调用 {name}...")
            print("=" * 80)
            result = await self.call_remote_agent(url, task_instruction, timeout=300)
            results[name] = result
        ...
        for name, result in results.items():
            status = "❌ 失败" if not result else "✅ 成功"
            print(f"\n{name}:")
            print(f"  状态: {status}")
            if result:
                # 尝试提取进程数
                if "个" in result or "process" in result.lower():
