...
def _should_load_agent(
    agent_name: str, mode: str, whitelist: Optional[list], blacklist: Optional[list]
) -> bool:
    """
    判断是否应该加载指定的 agent
    ...
    if mode == "all":
        ...
        # 未知模式，默认加载
        print(
            f"[Loader] Warning: Unknown capabilities mode '{mode}', defaulting to 'all'"
        )
        return True
...
def load_capabilities():
    ...
    # 3. Register Shared Memory I/O Tools (Level 1 - 4K circular buffer)
    from src.runtime_tools.memory_io_tools import (
        MemoryWriteTool,
        MemoryReadTool,
        MemoryClearTool,
        MemoryInfoTool,
    ...
    global_registry.register(MemoryWriteTool(), is_runtime_tool=True)
    global_registry.register(MemoryReadTool(), is_runtime_tool=True)
    global_registry.register(MemoryClearTool(), is_runtime_tool=True)
    global_registry.register(MemoryInfoTool(), is_runtime_tool=True)

    # 4. Register Shared File System Tools (Level 2 - .OneAgent directory)
    # NOTE: Not marked as runtime tools so ALL agents can use them
    ...
    global_registry.register(SharedSaveToFileTool())
    global_registry.register(SharedReadFileTool())
    global_registry.register(SharedListFilesTool())
    global_registry.register(SharedDeleteFileTool())

    from src.runtime_tools.redirect_wrapper_tools import (
        RedirectToFileTool,
        RedirectToMemTool,
        RedirectToFileAndMemTool,
    )

    global_registry.register(RedirectToFileTool())
    global_registry.register(RedirectToMemTool())
    global_registry.register(RedirectToFileAndMemTool())
    ...
    )  # Ensure capabilities package is importable
    ...
    for agent_dir in agents_root.iterdir():
        if agent_dir.is_dir() and (agent_dir / "agent.py").exists():
            ...
            if not _should_load_agent(
                ...
                print(
                    f"[Loader] Skipping agent: {agent_dir_name} (filtered by {capabilities_mode} mode)"
                )
                continue
                    ...
                    if (
                        ...
                        if tools_dir.exists():
                            for tool_file in tools_dir.glob("*.py"):
                                ...
                                try:
                                    ...
                                    for t_name, t_obj in inspect.getmembers(
                                        tool_module
                                    ):
                                        if (
                                            inspect.isclass(t_obj)
                                            and issubclass(t_obj, BaseTool)
                                            and t_obj is not BaseTool
                                        ):
                                            print(
                                                f"  [Loader] Found Scoped Tool: {t_name}"
                                            )
                                            tool_instance = t_obj()
                                    ...
                                    print(
                                        f"  [Loader] Failed to load tool {tool_file.name}: {e}"
                                    )
                        ...
                        if mcp_enabled:
                            ...
                            if mcp_config_path.exists():
                                ...
                                try:
                                    with open(
                                        mcp_config_path, "r", encoding="utf-8"
                                    ) as f:
                                        ...
                                        print(
                                            f"  [Loader] Loaded MCP config for {agent_instance.name}"
                                        )
                                except Exception as e:
