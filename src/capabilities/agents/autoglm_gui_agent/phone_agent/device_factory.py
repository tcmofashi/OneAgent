...
class DeviceFactory:
    ...
    def get_screenshot(self, device_id: Optional[str] = None, timeout: int = 10):
        """Get screenshot from device."""
        return self.module.get_screenshot(device_id, timeout)

    def get_current_app(self, device_id: Optional[str] = None) -> str:
        """Get current app name."""
        return self.module.get_current_app(device_id)

    def tap(
        self, x: int, y: int, device_id: Optional[str] = None, delay: Optional[float] = None
    ):
        """Tap at coordinates."""
    ...
    def back(self, device_id: Optional[str] = None, delay: Optional[float] = None):
        """Press back button."""
        return self.module.back(device_id, delay)

    def home(self, device_id: Optional[str] = None, delay: Optional[float] = None):
        """Press home button."""
        return self.module.home(device_id, delay)

    def launch_app(
        self, app_name: str, device_id: Optional[str] = None, delay: Optional[float] = None
    ) -> bool:
        """Launch an app."""
        return self.module.launch_app(app_name, device_id, delay)

    def type_text(self, text: str, device_id: Optional[str] = None):
        """Type text."""
        return self.module.type_text(text, device_id)

    def clear_text(self, device_id: Optional[str] = None):
        """Clear text."""
        return self.module.clear_text(device_id)

    def detect_and_set_adb_keyboard(self, device_id: Optional[str] = None) -> str:
        """Detect and set keyboard."""
        return self.module.detect_and_set_adb_keyboard(device_id)

    def restore_keyboard(self, ime: str, device_id: Optional[str] = None):
        """Restore keyboard."""
...
_device_factory: Optional[DeviceFactory] = None
