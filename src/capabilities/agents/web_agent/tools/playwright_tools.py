    ...
    async def execute(self, selector: str) -> str:
        try:
            ...
            if count > 1:
                ...
                for i in range(min(count, 5)):  # 最多显示前5个
                    ...
                    try:
                        ...
                        text = await el.evaluate("el => el.innerText?.substring(0, 50) || ''")
            ...
            try:
                ...
                handle = await page.wait_for_selector(selector, state="attached", timeout=2000)
                if handle:
...
Use this instead of get_page_info when you only need the text content."""
    ...
    async def execute(self, include_links: bool = False, include_tables: bool = True) -> str:
        try:
            import trafilatura
            ...
            return "Error: trafilatura library not installed. Run: pip install trafilatura"
        except Exception as e:
